...
//  TestCollectionView
//
//  Created by Even_cheng on 04/09/19.
...
open class BasePageCollectionCell: UICollectionViewCell {
    ...
    //打开之后的位移高度
    let openOffset: CGFloat = 65.0
    /// DEPRECATED! Animation oposition offset when cell is open
    @IBInspectable open var yOffset: CGFloat = kWindowBounds.width*0.075
...
extension BasePageCollectionCell {
    ...
    public func cellIsOpen(_ isOpen: Bool, animated: Bool = true) {
        ...
        if ySpacing == .greatestFiniteMagnitude {
            frontConstraintY.constant = isOpen == true ? -openOffset : 0
            backConstraintY.constant = isOpen == true ? openOffset - yOffset / 2 : 0
        } else {
        ...
        if let heightConstant = backContainerView.getConstraint(.height) {
            if additionalHeight == .greatestFiniteMagnitude {
                heightConstant.constant = isOpen == true ? frontContainerView.bounds.size.height + yOffset : frontContainerView.bounds.size.height-1
            } else {
                heightConstant.constant = isOpen == true ? frontContainerView.bounds.size.height + additionalHeight : frontContainerView.bounds.size.height-1
            }
        }
