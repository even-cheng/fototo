...
extension TransitionDriver {
    ...
    func pushTransitionAnimationIndex(_ currentIndex: Int,
        ...
        effectView.alpha = 0
        effectView.tag = 9999
        effectView.frame = CGRect.init(x: 0, y: 0, width: kWindowBounds.size.width, height: kWindowBounds.size.height)
        if headerHeight == kWindowBounds.height {
            copyView.frontContainerView.addSubview(effectView)
        }
    ...
    func popTransitionAnimationContantOffset(_ offset: CGFloat, backImage: UIImage?) {
        ...
        let copyEffertView = copyCell.frontContainerView.viewWithTag(9999)
        let currentEffertView = self.currentCell?.frontContainerView.viewWithTag(9999)
        ...
        UIView.animate(withDuration: duration, delay: 0, options: UIView.AnimationOptions(), animations: {
            ...
            copyEffertView?.alpha = 0
            currentEffertView?.alpha = 0

        }, completion: { _ in
            copyEffertView?.removeFromSuperview()
            currentEffertView?.removeFromSuperview()
